...
function SearchPage({ user, onRequireLogin }: { user: User | null; onRequireLogin: () => void }) {
  ...
  return <main className="page"><section className="hero"><span className="eyebrow">BILI MARKET MONITOR · v{APP_VERSION}</span><h1>先看清价格，再决定要不要出手。</h1><p>粘贴 B 站市集商品的 ClsId，查询当前最低可购买价，并设置自己的目标价格提醒。</p><form className="search-form" onSubmit={search}><input aria-label="商品 ID" value={clusterId} onChange={e => { setClusterId(e.target.value); setMessage(""); }} placeholder="输入商品 ID，例如 10000002733" inputMode="numeric" aria-invalid={Boolean(message)} aria-describedby={message ? "search-error" : undefined} className={message ? "input-error" : ""} /><button className="primary" disabled={loading}>{loading ? "查询中…" : "查询商品"}</button></form>{message && <p id="search-error" className="error search-error" role="alert" aria-live="polite">{message}</p>}<p className="privacy-note">价格监控直接使用 B 站市集接口；查询结果会保留最近一次成功数据。</p></section>{product && <ProductCard product={product} user={user} onFavorite={favorite} onRefresh={refresh} /> }<section className="help-card"><div><span className="eyebrow">CLS ID 教程</span><h2>还没有商品 ID？先去 BiliMarketBoss 找到它</h2><p>在第三方网站搜索或收藏商品，复制商品信息中的 ClsId 纯数字，再粘贴回本站。本站查询结果也提供对应 B 站市集详情页入口，便于浏览确认。</p></div><a className="button primary" href={BOSS_URL} target="_blank" rel="noopener noreferrer">打开 BiliMarketBoss ↗</a></section>{!user && <section className="signin-prompt"><div><h3>保存收藏并接收提醒</h3><p>注册账户后可以收藏最多 20 件商品，并设置每件商品独立的检查频率。</p></div><button className="button outline" onClick={onRequireLogin}>登录 / 注册</button></section>}</main>;
}

function FavoritesPage({ user, onProductRefresh }: { user: User; onProductRefresh: () => void }) {
  const [items, setItems] = useState<Favorite[]>([]); const [history, setHistory] = useState<Record<string, HistoryPoint[]>>({}); const [message, setMessage] = useState("");
  ...
  useEffect(() => { void load(); }, []);
  async function update(item: Favorite, patch: Record<string, unknown>) { try { const next = await api<Favorite>(`/bili-market/favorites/${item.cluster_id}`, { method: "PATCH", body: JSON.stringify(patch) }); setItems(values => values.map(value => value.id === item.id ? next : value)); } catch (error) { setMessage(toUserMessage(error)); } }
...
function ProfilePage({ user, setUser }: { user: User; setUser: (user: User) => void }) {
  ...
  async function sendCode() { setMessage(""); try { await api("/profile/email/send-code", { method: "POST", body: JSON.stringify({ email }) }); setCooldown(60); setMessage("验证码已发送，请检查邮箱（开发环境未配置 SMTP 时不会实际发信）。"); } catch (error) { setMessage(toUserMessage(error)); } }
...
export default function App() {
  const [user, setUser] = useState<User | null>(null); const [page, setPage] = useState(window.location.pathname === "/reset-password" ? "reset-password" : "home"); const [authMode, setAuthMode] = useState<"login" | "register" | "forgot">("login");
  useEffect(() => { void api<{ user: User }>("/auth/me").then(data => setUser(data.user)).catch(() => undefined); }, []);
  async function logout() { await api("/auth/logout", { method: "POST" }); setUser(null); setPage("home"); }
  function login() { setAuthMode("login"); setPage("login"); }
  return <><Header user={user} page={page} setPage={setPage} onLogout={() => void logout()} />{page === "home" && <SearchPage user={user} onRequireLogin={login} />}{page === "login" && <main className="page auth-page"><AuthPanel mode={authMode === "register" ? "register" : "login"} onSuccess={value => { setUser(value); setPage("home"); }} setMode={mode => { setAuthMode(mode); if (mode === "forgot") setPage("forgot"); }} /></main>}{page === "forgot" && <main className="page auth-page"><ForgotPanel setMode={mode => { setAuthMode(mode); setPage(mode === "forgot" ? "forgot" : "login"); }} /></main>}{page === "reset-password" && <main className="page auth-page"><ResetPasswordPage onDone={() => { setPage("login"); setAuthMode("login"); }} /></main>}{page === "favorites" && user && <FavoritesPage user={user} onProductRefresh={() => undefined} />}{page === "profile" && user && <ProfilePage user={user} setUser={setUser} />}{page === "admin" && user?.role === "admin" && <AdminPanel />}<footer><span>B站市集好价提示系统 · Bili Market Monitor · v{APP_VERSION}</span><a href={BOSS_URL} target="_blank" rel="noopener noreferrer">BiliMarketBoss 商品检索入口</a></footer></>;
}
